#!/usr/bin/env python3
"""
Edge Equation — NHL Historical Lines Backfill
=============================================
Pulls one historical Odds-API snapshot per game-day for every NHL
season we already have games.json for. Persists per-season into
`data/backfill/nhl/<season>/lines.json`.

Costs 10 credits per call (per The Odds API $30/mo plan). Default
3-season pull = ~600 calls = ~6,000 credits. Budget-safe — single-month
pull stays under the 20K monthly cap.

Idempotent: re-runs skip game-days already covered.

Usage:
    ODDS_API_KEY=... python run_nhl_lines_backfill.py
    ODDS_API_KEY=... python run_nhl_lines_backfill.py --seasons 2023,2024,2025
    ODDS_API_KEY=... python run_nhl_lines_backfill.py --seasons 2023-2025
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="NHL multi-season historical lines backfill (Odds API)",
    )
    parser.add_argument(
        "--seasons", type=str, default="2023-2025",
        help="Comma-separated list or range. Default: 2023-2025.",
    )
    parser.add_argument(
        "--snapshot-hour-utc", type=int, default=22,
        help="UTC hour to snapshot each game-day at (0-23). Default 22 = 6pm ET.",
    )
    parser.add_argument(
        "--output-dir", type=str, default=str(DEFAULT_OUTPUT_DIR),
    )
    parser.add_argument(
        "--quota-log", type=str, default=str(DEFAULT_QUOTA_LOG),
    )
