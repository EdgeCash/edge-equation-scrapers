#!/usr/bin/env python3
"""
Edge Equation — WNBA Historical Lines Backfill
==============================================
Pulls one historical Odds-API snapshot per game-day for every WNBA
season we already have games.json for. Persists per-season into
`data/backfill/wnba/<season>/lines.json`.

WNBA seasons are calendar-year (mid-May → October). Far fewer game-
days than NBA (~110 vs ~200 per season), so credit cost is lower.

Costs 10 credits per call. Default 3-season pull = ~330 calls =
~3,300 credits.

Idempotent: re-runs skip game-days already covered.

Usage:
    ODDS_API_KEY=... python run_wnba_lines_backfill.py
    ODDS_API_KEY=... python run_wnba_lines_backfill.py --seasons 2023-2025
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="WNBA multi-season historical lines backfill (Odds API)",
    )
    ...
    print(f"\nWNBA Historical Lines Backfill")
